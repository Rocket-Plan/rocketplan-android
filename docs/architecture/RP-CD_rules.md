...
Violations: RP-BUG-035 (WorkScope, fixed). Candidate per-scope pulls traced 2026-06-07 and cleared of *this* rule (no pending-create drop): **damages** and **albums** (no offline-create path exists — both are server-sourced only, so the scenario is moot); **support conversations/messages** (offline creates exist, but the pull `@Upsert` matches on a local PK so the pending row is not dropped). NB: the support pull has a *separate* duplicate-on-refresh defect (server returns its own `uuid`; pull does no `serverId` reconciliation → a second row per server id) — that is a distinct failure mode, not an RP-CD-014 drop.
