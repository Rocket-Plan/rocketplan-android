...
| `RP-FR-030` | P3 | — | RP-FR-019 has no **placement date-correction** — Android cannot fix a wrong `date_in`/`date_out` on a placement (no endpoint wired, no UI). iOS added the backend `PATCH /api/equipment-asset-placements/{id}` + `SerializedPlacementEditView` (UTC date-only correction, RP-BUG-345 fixed an off-by-one). Verify the endpoint exists on the current backend spec before wiring (API Contract Discipline) | feature | new_feature | iOS parity review 2026-07-18 | feat/RP-FR-019-serialized-equipment | fixed | unreleased | — | [RP-FR-030](investigations/RP-FR-030_no_placement_date_correction_2026-07-18.md) · iOS `SerializedPlacementEditView` / `correctPlacement` · parent [RP-FR-019](plans/plan_rp_fr_019_serialized_equipment_2026-07-13.md) · [plan](plans/plan_rp_fr_030_placement_date_correction_2026-07-18.md) |
| `RP-FR-031` | P3 | — | RP-FR-019 has no **placement delete** — Android cannot remove an erroneous placement record (no endpoint wired, no UI). iOS added the backend `DELETE /api/equipment-asset-placements/{id}` + swipe-to-delete in the history view. Verify the endpoint exists on the current backend spec before wiring (API Contract Discipline) | feature | new_feature | iOS parity review 2026-07-18 | feat/RP-FR-019-serialized-equipment | fixed | unreleased | — | [RP-FR-031](investigations/RP-FR-031_no_placement_delete_2026-07-18.md) · iOS `deletePlacement` (RP-BUG-344) · parent [RP-FR-019](plans/plan_rp_fr_019_serialized_equipment_2026-07-13.md) · [plan](plans/plan_rp_fr_031_placement_delete_2026-07-18.md) |
| `RP-FR-032` | P3 | — | RP-FR-019 serialized-equipment screens have **interim/missing navigation entry points** — the company pool (`SerializedEquipmentPoolFragment`, RP-FR-026) is only reachable via an undiscoverable **long-press** on the equipment-totals button, and the room-list rows have **no affordance to open the per-unit detail screen** (RP-FR-027) — the row tap/buttons are claimed by edit + lifecycle actions. Add a proper, discoverable pool entry point (equipment tab/menu) and a room-row → asset-detail affordance. iOS mounts pool + room-assets cleanly via the room/totals mode gate | feature | new_feature | iOS parity review 2026-07-19 | — | open | unreleased | — | [RP-FR-032](investigations/RP-FR-032_serialized_equipment_entry_points_2026-07-19.md) · follow-up of [RP-FR-026](investigations/RP-FR-026_no_company_asset_pool_screen_2026-07-18.md)/[RP-FR-027](investigations/RP-FR-027_no_asset_detail_screen_2026-07-18.md) · parent [RP-FR-019](plans/plan_rp_fr_019_serialized_equipment_2026-07-13.md) |
| `RP-HD-010` | P2 | — | RP-FR-019 serialized-equipment offline path is **not yet on-device-verified and lacks iOS's offline hardening depth** — the six new screens (RP-FR-026..031) plus the sync spine pass unit + build gates only; none has been driven on an enabled-company device. iOS earned its robustness through a long tail of shipped offline fixes (RP-BUG-344..363: offline caching, temp-ID remap, replay reconcile, room-ID promotion, date-preservation, project labeling). Audit the Android serialized offline path against those lessons + run an on-device offline E2E pass (register/deploy/move/check-out/correct/delete offline → sync → refresh, incl. process-death + multi-company). Complements RP-FR-019's outstanding on-device E2E item | functional | pre_existing_latent | iOS parity review 2026-07-19 | — | open | unreleased | — | [RP-HD-010](investigations/RP-HD-010_serialized_offline_hardening_ondevice_2026-07-19.md) · vs iOS RP-BUG-344..363 · parent [RP-FR-019](plans/plan_rp_fr_019_serialized_equipment_2026-07-13.md) |
