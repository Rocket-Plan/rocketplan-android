...
| `RP-FR-018` | P2 | — | Equipment Move & Tracking (Android client) — move equipment room→room (partial qty) and transfer project→project, with movement history; consumes backend MONGOOSE-FR-014 move/transfer/movements endpoints (count-based, full ledger, strictly additive). Adds MOVE/TRANSFER sync op types + payload-remap coverage for the destination room/project. **Depends on RP-BUG-279** (pivot write-sync realignment) landing first | feature | new_feature | spec 2026-07-09 | — | planned | unreleased | — | [plan](plans/plan_rp_fr_018_equipment_move_and_tracking_2026-07-09.md) · fixes plan [H1–H4+M2 + Review Round 2](plans/plan_rp_fr_018_h1_h4_m2_fixes_2026-07-10.md) (H1–H4/M2 + R1–R3 applied in working tree, uncommitted; unit tests pending) · backend `mongoose:MONGOOSE-FR-014` · depends `RP-BUG-279` · siblings iOS `RP-FR-023` / Webapp `WEBAPP-FR-004` · **consume backend `is_active` (MONGOOSE-BUG-042, 2026-07-12):** moved-out placements are now returned as *ended* records (`GET rooms/{room}/equipment` → `is_active:false`), not hidden — render them as **locked historical rows** (show History, hide Move/Edit) and gate Move on `is_active` (server 422s a move of an ended placement: `"The source equipment placement has already ended and cannot be moved."`); decode an optional `is_active` on the equipment pivot, treat missing as active |
...
| `RP-BUG-279` | P1 | — | Offline equipment write-sync targets non-existent backend routes (PUT/DELETE /api/equipment/{id}) — creates/edits/deletes silently dropped (404→recreate→duplicate-name 422→drop; delete 404→no-op) and never round-trip to the server's equipment_room placement model, so iOS/webapp never see Android's equipment; must adopt the pivot endpoints (attach via POST rooms/{r}/equipment, update/delete via equipment-rooms/{id}, read via GET rooms/{r}/equipment) | functional | new_code_bug | 1.30 (35) | — | planned | unreleased | — | [plan](plans/plan_rp_bug_279_equipment_writesync_pivot_realignment_2026-07-09.md) · pivot endpoints (attach/update/delete/move/transfer/movements) adopted in working tree (uncommitted); verify each path/wrapper against backend `docs/openapi.yaml` before commit · backend `mongoose:MONGOOSE-FR-014` §8 · **backend prerequisite `mongoose:MONGOOSE-BUG-036`** (attach must accept/return pivot uuid/date_in/quantity) · prerequisite for `RP-FR-018` · OfflineSyncApi.kt:490 |
