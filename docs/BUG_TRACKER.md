...
| `RP-FR-009` | P2 | — | Floorplan sketching — canvas editor (freehand draw → straighten to lines → resize rooms by editing wall lengths), append-only revisions + restore, room binding to real Rooms; consumes backend Floorplan API; gated behind `floorplan_enabled` | feature | new_feature | spec 2026-06-14 | — | planned | unreleased | — | [RP-FR-009](investigations/RP-FR-009_floorplan_sketching.md) · sibling iOS RP-FR-016 · GH #5 |
| `RP-FR-017` | P3 | — | **Capture user GPS location at login (debugging).** Attach the device's current/last-known coordinates to the remote-log batch payload so `log_entries` records where a user was when they logged on — parity with iOS `RP-FR-022`; supports the auth/login debugging cluster. **Consent posture (required):** capture **only when `ACCESS_FINE_LOCATION`/`ACCESS_COARSE_LOCATION` is already granted**; **never** request location permission from the login flow (intrusive + Play-Store scrutiny). When not granted, send no coordinates (backend stores NULL). Reuse existing location infra (FusedLocationProvider / last-known). Batch-level fields on the `/api/logs/ios` payload: `latitude`, `longitude`, `location_accuracy` (meters), `location_captured_at` (ISO8601). Backend done — pairs with `mongoose:MONGOOSE-FR-012`, iOS `RP-FR-022`. | feature | new_feature | spec 2026-07-01 | — | planned | unreleased | — | [RP-FR-017](investigations/RP-FR-017_login_gps_capture_2026-07-01.md) · backend `mongoose:MONGOOSE-FR-012` · sibling iOS `RP-FR-022` |
