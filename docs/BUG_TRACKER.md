...
| `RP-BUG-040` | P2 | — | Offline delete of a server-modified entity fails permanently — delete handlers send a stale updated_at, backend assertNotStale returns 409, and they retry with the same stale timestamp without re-fetching a fresh one (Note/AtmosphericLog/Room/Location/Property/Equipment/MoistureLog; Photo safe) | functional | pre_existing_latent | 409 sweep 2026-06-07 (backend verified) | — | open | unreleased | — | [RP-BUG-040](investigations/RP-BUG-040_delete_409_stale_timestamp_retry_loop.md) · rule RP-CD-005 |
